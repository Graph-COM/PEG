...
def create_dataloader_source(association, val_ratio = 0.1, seed = 0):
    ...
    val_index,train_index=data_split(asso_index, val_ratio)
    ...
    return train_dataset, val_dataset, train_matrix, train_edge_index


def create_dataloader_target(association, test_ratio, seed = 0):
    adj_triu = np.triu(association)
    none_zero_position=np.where(adj_triu==1)
    none_zero_row_index=none_zero_position[0]
    none_zero_col_index=none_zero_position[1]
...
def create_dataloader_source_plus(association, val_ratio = 0.1, seed = 0):
    adj_triu = np.triu(association)
    none_zero_position=np.where(adj_triu==1)
    none_zero_row_index=none_zero_position[0]
    none_zero_col_index=none_zero_position[1]
    ...
    val_dataset = lkpDataset(root='data', dataset='data/' + '_val',id_map=id_val, label = val_label)
    return val_dataset, train_matrix, train_edge_index, id_train_positive, id_train_negative

def create_dataloader_target_plus(association, test_ratio, seed = 0):
    adj_triu = np.triu(association)
    none_zero_position=np.where(adj_triu==1)
    none_zero_row_index=none_zero_position[0]
    none_zero_col_index=none_zero_position[1]
...
def data_split(full_list, ratio1, shuffle=True):
